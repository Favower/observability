...
// SendMetric - метод для отправки метрик в формате JSON через HTTP.
func (s *HTTPMetricSender) SendMetric(metricType, metricName string, value float64) error {
	...
	// Обработка значений в зависимости от типа метрики.
	if metricType == "gauge" {
		reqBody.Value = &value
	} else if metricType == "counter" {
		reqBody.Delta = new(int64)
		*reqBody.Delta = int64(value)
	}
...
// CollectAndSendMetrics - метод для сбора метрик и их отправки через указанный MetricSender.
...
// collectMetrics - метод для сбора метрик о состоянии системы.
